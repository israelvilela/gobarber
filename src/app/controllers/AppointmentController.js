...
class AppointmentController {
  ...
  async store(req, res) {
    ...
    /**
     * Verifica se o provider está marcando para ele mesmo
     */
    if (provider_id === req.userId) {
      return res
        .status(400)
        .json({ message: 'You can not appoint for yourself.' });
    }
    ...
    if (isBefore(hourStart, new Date())) {
      return res.status(400).json({ message: 'Past date is not permitted' });
    }
